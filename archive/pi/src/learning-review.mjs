...
import { classifyVaultTemporalRequest, parseVaultTemporalRequest } from './temporal-query.mjs';

export const learningReviewLimits = Object.freeze({
  // These are the original Second Mind review ceilings. They bound a
  // server-owned plan; Pi receives one tool-free generation at a time and can
  // never decide what to read next.
  maxTurns: 50, timeoutMs: 30 * 60_000, retrievalTimeoutMs: 24 * 60_000,
  maxReadSearchCalls: 40,
  maxBatchChars: 24_000, maxBatches: 49, maxTotalChars: 960_000,
  concurrency: 2, readConcurrency: 8,
  // The prior migration's 180s / 12K cap truncated xhigh extraction before
  // evidence could be returned. Use the configured request ceiling (capped
  // again by the router) and original Claude output ceiling; never raise the
  // shared 30-minute / 50-call budget or add autonomous continuation.
  extractionTimeoutMs: 600_000, extractionOutputTokens: 131_072,
  maxRelatedQueries: 8, maxRelatedDocuments: 24, maxFactChars: 120_000,
});

const ALL_SCOPE = /^(?:所有(?:的)?|全部(?:的)?|都(?:要|总结)?|所有方向|全部方向|各个方向|all(?:\s+of\s+them)?)[。.!！\s]*$/iu;
const LEARNING = /学习|学了|学过|复习|读书|阅读|读了|读过|做了|做过|完成|推进|成果|课程|learn|stud(?:y|ied|ies)|reading/iu;
...
function reviewTimeZone(value) {
  const requested = String(value || 'Asia/Shanghai').trim();
  try { new Intl.DateTimeFormat('en', { timeZone: requested }).format(0); return requested; }
...
function monthStart(parts, delta, keepDay, timeZone) {
  const target = new Date(Date.UTC(parts.year, parts.month - 1 + delta, 1));
  ...
  return midnight(target.getUTCFullYear(), target.getUTCMonth() + 1,
    keepDay ? Math.min(parts.day, maxDay) : 1, timeZone);
}
function daysFrom(parts, delta, timeZone) {
  const target = new Date(Date.UTC(parts.year, parts.month - 1, parts.day + delta));
...
/** Preserve every date-window boundary supported by the original service. */
function originalLearningReviewRange(text, now, timeZone) {
  const zone = reviewTimeZone(timeZone);
  const parts = partsAt(now, zone);
  let startMs;
  let endMs = now;
  const explicit = text.match(/(\d{4})[-/年](\d{1,2})[-/月](\d{1,2})日?\s*(?:到|至|[~～—–])\s*(?:(\d{4})[-/年])?(\d{1,2})[-/月](\d{1,2})日?/u);
  if (explicit) {
    const [, year, month, day, endYear, endMonth, endDay] = explicit
      .map((part, index) => index ? Number(part) : part);
    ...
    endMs = Math.min(daysFrom({ year: finalYear, month: endMonth, day: endDay }, 1, zone) - 1, now);
  } else if (/(?:最近|过去|近|这)\s*(?:一|1)?个?月|last month|past month/iu.test(text)) {
    startMs = monthStart(parts, -1, true, zone);
  } else if (/上个?月/u.test(text)) {
    startMs = monthStart(parts, -1, false, zone);
    endMs = monthStart(parts, 0, false, zone) - 1;
  } else if (/本月|这个月/u.test(text)) {
    startMs = monthStart(parts, 0, false, zone);
  } else if (/本周|这周|上周|上个星期/u.test(text)) {
    const weekday = new Date(Date.UTC(parts.year, parts.month - 1, parts.day)).getUTCDay();
    const back = (weekday + 6) % 7;
    const last = /上周|上个星期/u.test(text);
    startMs = daysFrom(parts, -back - (last ? 7 : 0), zone);
    if (last) endMs = daysFrom(parts, -back, zone) - 1;
  } else if (/(?:最近|过去|近)\s*(?:一|1)?(?:周|个星期)/u.test(text)) {
    startMs = daysFrom(parts, -7, zone);
  } else if (/今天|今日|昨天|昨日/u.test(text)) {
    const yesterday = /昨天|昨日/u.test(text);
    startMs = daysFrom(parts, yesterday ? -1 : 0, zone);
    if (yesterday) endMs = daysFrom(parts, 0, zone) - 1;
  ...
  return {
    ...
    endExclusive: new Date(endMs + 1).toISOString(),
    timeZone: zone,
  };
}
...
export function normalizeLearningReview(value) {
  if (!value || value.kind !== 'learning_review') return null;
...
export function resolveLearningReviewRequest(question, options = {}) {
  ...
  if (!PERSONAL.test(raw) && !/(?:总结|回顾|盘点|汇总).{0,35}学习(?:的)?(?:重点|内容|情况|进展)|(?:最近|近来|过去|本月|上月|本周|上周).{0,30}(?:学习|学(?:过|了)?|复习|读(?:过|了)?|做(?:过|了)?|完成|推进|成果|课程)/u.test(raw)) return null;
  const now = Number(options.now instanceof Date ? options.now.getTime() : options.now ?? Date.now());
  if (!Number.isFinite(now)) return null;
  // Preserve the original fail-closed contract for an explicitly relative
  // period we cannot translate. In particular, the generic “近期” fallback
  // must not silently reinterpret “最近一个季度” as “最近一个月”.
  const temporal = classifyVaultTemporalRequest(raw, { now, timeZone: options.timeZone });
  if (temporal.matched && !temporal.supported) return null;
  const range = originalLearningReviewRange(raw, now, options.timeZone) ||
    parseVaultTemporalRequest(`我的知识库 ${raw}`, { now, timeZone: options.timeZone })?.range;
  if (!range) return null;
  return normalizeLearningReview({
    kind: 'learning_review', originalQuestion: raw, scope: 'all',
    range, capturedAt: new Date(now).toISOString(),
  });
}
...
function evidenceDateRange(excerpt, years) {
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    // A reference to the previous plan is not the current activity's date.
    if (/(?:承接|接续|参考|参照|引用)\s*$/u.test(prefix)) continue;
    // “昨日（09-02）的剩余已并入今日” dates the carried-over work,
    // not today's plan. An actual “昨日（09-02）已完成” remains dated.
    if (/(?:昨日|昨天)\s*[（(]?\s*$/u.test(prefix) &&
        /^[）)]?[^。；;\n]{0,80}(?:并入|顺延到|挪至|转入|补到)(?:今日|今天|当日)/u.test(suffix)) continue;
...
export function validateLearningReviewFacts(output, batch, review) {
  ...
  for (const item of input.slice(0, 100)) {
    ...
    for (const proposed of Array.isArray(item?.evidence) ? item.evidence.slice(0, 12) : []) {
      ...
      const evidenceItem = { segmentId: segment.id, path: segment.path, lineStart: start, lineEnd: end, quote };
      ...
      if (segment.dateBasis !== 'related' && ['plan', 'diary', 'activity'].includes(segment.recordType) && overlaps(segment.dateRange, window)) {
        ...
        if (segment.yearBasis === 'request_window' && !evidenceDateRange(excerpt, [])) {
          unresolvedYear = true;
        } else primary.push({ segment, quote, excerpt, evidenceItem });
      }
    }
    ...
    const explicit = [];
    const evidenceDates = primary.flatMap((entry) => {
      const range = evidenceDateRange(entry.excerpt, [Number(entry.segment.dateRange.start.slice(0, 4))]);
      if (range) explicit.push(range.start, range.end);
      const dated = range || entry.segment.dateRange;
      entry.evidenceItem.dateRange = { start: dated.start, end: dated.end };
      return [dated.start, dated.end];
    });
    start = evidenceDates.sort()[0]; end = evidenceDates.at(-1);
    ...
    // An unchecked checkbox describes the whole task, not the absence of
    // progress. Explicit actual-work receipts with remaining work are evidence
    // of partial progress; a future target such as “计划今日完成” is not.
    const partialProgress = /剩余|顺延|还剩|未完/u.test(contexts) &&
      [...contexts.matchAll(/(?:今日|当日|当天|今天|实际|已经|已)(?:已)?(?:完成|学习|阅读|复习)(?:了)?\s*\d+(?:\.\d+)?\s*(?:小时|分钟|页|章|节)/gu)]
        .some((match) => !/(?:计划|预计|希望|打算|尚未|没有|未)\s*$/u.test(contexts.slice(0, match.index)));
    const completed = !explicitlyPlanned && !denied && /\[[xX]\]|(?:已|彻底|全部|正式)(?:经)?(?:完成|做完|读完|看完|结束|复习)|(?:完成|做完|读完|看完|掌握)了|(?:学习|阅读|复习)(?:并|和)(?:整理|总结)了|已读|读过|学过|复习过|✅/u.test(contexts);
    const ongoing = partialProgress || (!explicitlyPlanned && !denied && /进行中|正在|继续|推进|学习了|学了|读了|复习了|阅读了/u.test(contexts));
    ...
    const statesPartialProgress = partialProgress && /\d+(?:\.\d+)?\s*(?:小时|分钟|页|章|节)|部分/u.test(statement) &&
      !/全部|彻底|整体|整个/u.test(statement);
    if (!completed && !statesPartialProgress && /已(?:经)?(?:完成|掌握|结束|做完|读完)|彻底完成|完成了|做完了|掌握了|读完了/u.test(statement) &&
    ...
    if (partialProgress) status = 'in_progress';
...
export const learningReviewInternals = {
  dateExpression, fullDates, validDay, windowDays, recordType, originalLearningReviewRange, DAY_MS,
};

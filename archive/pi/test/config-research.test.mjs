...
test('research and outbound page-reading features default off with bounded limits', () => {
  ...
  assert.equal(config.llm.contextWindow, 1_000_000);
  assert.deepEqual(config.webSearch.officialDomains, []);
});

test('Pi context window accepts only bounded integer token counts', {
  concurrency: false,
}, () => {
  const name = 'LLM_CONTEXT_WINDOW';
  const previous = process.env[name];
  try {
    process.env[name] = '131072';
    assert.equal(createConfig().llm.contextWindow, 131_072);
    for (const invalid of ['4095', '2000001', '262144.5', 'not-a-number']) {
      process.env[name] = invalid;
      assert.throws(() => createConfig(), /LLM_CONTEXT_WINDOW must be an integer/u);
    }
  } finally {

...
const REVIEW_EVIDENCE_RULES = Object.freeze([
  '本期活动与成果只能由已读取的期内日记、计划或周计划中的明确活动句建立。每一项必须紧邻给出：事件日期、记录类型、原文逐字短引文，以及该日期记录的来源链接。逐字短引文不得改写或补全；计划句只能支持本期计划，不能支持已学习或已完成。',
  '学习笔记的标题日期、frontmatter 日期、图片文件名、截图时间、附件时间及文件修改时间均不是学习活动日期，也不能证明本期产出。文件元数据绝不建立学习活动或成果；日期标题只用于定位待核验段落，不能代替段落中的活动句。不得编造未读取或根本不存在的图片、附件或时间。',
  '旧笔记或无活动记录的笔记只能单独放在“关联资料说明”，用于解释已有期内事件，不得称为本期新增、本期产出或本期完成。无法给出期内活动句及逐字短引文的资料移到“未确认资料列表”，注明时间归属未确认，不计入本期学习、成果或已完成数量。不能用“笔记中存在这些内容”推断本期学过这些内容。',
...
export function createLearningReviewToolBudget(task, { now = () => Date.now(), onLimit = () => {} } = {}) {
  ...
  const preToolUse = async (input) => {
    ...
    let context = state.closed
      ...
      : `服务端回顾工具预算：${state.calls}/${learningReviewExecutionBudget.maxToolCalls} 次。优先覆盖日期记录；独立短记录可在同轮并行 Read。`;
...
export function learningReviewPrompt(review) {
  ...
  return [
    ...
    '先使用 Glob/Grep 枚举全库的期内日记、计划、周计划和日期段落，建立覆盖清单，再按清单分批 Read；普通检索前几项不能代表覆盖全库。',
    '识别日期文件名、日期标题、周计划日期范围和日期表格列；混合日期文档只采用期内片段，月日需有明确年份上下文并通过日历检查，页码和比例不是日期。',
    '长文用 Read 的 offset/limit 连续分段；先覆盖期内记录，再沿明确引用及主题查读学习笔记和关联原文。只读过片段不能称已全文覆盖。',
    ...
    '每项“本期学习”必须有范围内的学习事件日期及已读取的原文证据，来源须就近引用。禁止把时间范围外的记录纳入本期；例如起点为 8 月 6 日，则 8 月 3—5 日不属于范围。',
    '文件修改时间只辅助发现资料，不能作为学习事件日期。旧笔记仅在期内记录明确写到学过、复习或实践时可纳入；仅主题相关（包括检索命中）不足以纳入。',

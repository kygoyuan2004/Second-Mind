...
class FakeEmbeddingClient {
  ...
  async rerank(query, documents, options = {}) {
    this.rerankCalls.push({ query, documents: [...documents], topN: options.topN });
    return documents.slice(0, options.topN).map((_, index) => ({ index, score: 1 - index / 100 }));
  }
}

test('automatic routing, follow-up context and conditional reranking match the original contract', async (t) => {
  assert.deepEqual(routeKnowledgeQuery('列出所有包含 `queryVector` 的文件'), {
    route: 'exact', exhaustive: true,
  });
  assert.deepEqual(routeKnowledgeQuery('苹果为什么是红色水果？'), {
    route: 'semantic', exhaustive: false,
  });

  const setup = await fixture(t, {
    provider: 'openai-compatible', model: 'fake-three-dimensional', dimensions: 3,
  });
  await Promise.all([
    fsp.writeFile(path.join(setup.vaultPath, 'apple.md'), '# 苹果笔记\n\n苹果是一种红色水果。\n'),
    fsp.writeFile(path.join(setup.vaultPath, 'banana.md'), '# 香蕉笔记\n\n香蕉是一种黄色水果。\n'),
  ]);
  const client = new FakeEmbeddingClient();
  const index = new KnowledgeIndex(setup.config, { client });
  t.after(() => index.close());
  await index.ready;
  const result = await index.search('这个呢？', {
    taskMode: 'normal',
    previousUserQuestion: '苹果为什么是红色水果？',
    previousAnswerTitles: ['苹果笔记'],
  });
  assert.equal(result.route, 'semantic');
  assert.match(result.query, /Previous question: 苹果为什么是红色水果/u);
  assert.match(result.query, /Previous answer titles: 苹果笔记/u);
  assert.equal(result.diagnostics.rerankerRequested, true);
  assert.equal(result.diagnostics.rerankerUsed, true);
  assert.equal(client.rerankCalls.length, 1);
...
test('a corrupt current generation falls back atomically while ordinary retrieval overlays the live Vault', async (t) => {
  const setup = await fixture(t);
  ...
  const live = await recovered.search('second generation', { route: 'keyword' });
  assert.match(live.results[0].content, /second generation/);

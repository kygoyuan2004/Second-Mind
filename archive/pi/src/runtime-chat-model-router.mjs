...
import { generateWithPi } from './pi-generation-executor.mjs';
import {
...
export class RuntimeChatModelRouter {
  constructor(options = {}) {
    ...
    this.baseConfig = Object.freeze({
      ...
      contextWindow: finiteInteger(
        options.baseConfig?.contextWindow,
        DEFAULT_PI_CONTEXT_WINDOW_TOKENS,
        MIN_PI_CONTEXT_WINDOW_TOKENS,
        MAX_PI_CONTEXT_WINDOW_TOKENS,
      ),
    ...
    // A custom client factory exists solely for isolated transport fixtures.
    // Production construction never supplies it and is therefore always Pi.
    this.piGenerate = options.piGenerate || (options.clientFactory ? null : generateWithPi);
  ...
  createLease(snapshot, modelId) {
    ...
    const piBinding = () => {
      const value = {
        ...
        maxOutputTokens: maximumOutputTokens,
        // The managed catalog does not yet claim a verified per-model context
        // window, so Pi uses the instance-level operator setting by default.
        contextWindow: finiteInteger(
          binding.model.contextWindow,
          this.baseConfig.contextWindow,
          MIN_PI_CONTEXT_WINDOW_TOKENS,
          MAX_PI_CONTEXT_WINDOW_TOKENS,
        ),
    ...
    const generate = (messages, options = {}) => {
      ...
      const generationOptions = {
        ...
        maxOutputTokens,
        timeoutMs: Math.min(
          finiteInteger(options.timeoutMs, this.baseConfig.timeoutMs, 1_000, 900_000),
          this.baseConfig.timeoutMs,
        ),
      };
      return this.piGenerate
        ? this.piGenerate(piBinding(), messages, generationOptions)
    ...
    const lease = {
      ...
      usesPiExecutor: Boolean(this.piGenerate),
      model: binding.model,
    ...
    const results = await mapWithConcurrency(enabled, concurrency, async (model) => {
      ...
      try {
        ...
        const result = Object.freeze({
          modelId,
          ok: true,
          code: 'PI_GENERATION_VERIFIED',
          message: '',
          capability: 'pi-single-generation',
        });

...
按用户要求，原版与 Second Mind 同步取消学习回顾中“必须由日记或计划活动句证明成果”的门槛。技术笔记、整理笔记及其正文或日期属性可直接作为回顾依据，回答需注明实际日期来源；日期待核的相关资料可以列出。当前规则也注入普通问答，使续聊不沿用旧门槛。首轮、阶段提醒和预算收尾采用同一规则。

日期范围、来源引用、只读权限、覆盖统计和执行预算保持不变；计划不等于完成，整理或修改日期不等于首次创建日期。此次调整不提供按文件修改时间穷举的工具。下文及原始源码指纹保留迁移时的历史基线，不代表此次调整后的源码逐字相同。

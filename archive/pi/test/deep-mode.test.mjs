...
test('Deep mode plans multiple queries, expands hybrid retrieval, fuses evidence, and persists mode', async (t) => {
  ...
  assert.deepEqual(
    fixture.searches.map((item) => item.options.taskMode),
    ['deep', 'deep', 'deep'],
  );
  assert.ok(fixture.searches.every((item) => item.options.limit === undefined));
  assert.ok(fixture.searches.every((item) => item.options.route === undefined));
...
test('Normal mode remains a single retrieval and generation pass', async (t) => {
  ...
  assert.equal(fixture.searches[0].options.taskMode, 'normal');
  assert.equal(fixture.searches[0].options.limit, undefined);
  assert.equal(fixture.searches[0].options.route, undefined);
...
test('Deep uses a safe top-k fallback when an older programmatic config has no deep block', async (t) => {
  ...
  assert.equal(searches[0].taskMode, 'deep');
  assert.equal(searches[0].limit, undefined);
  assert.equal(searches[0].route, undefined);
});

...
import { VaultPathPolicy, isIndexable } from '../../../src/path-policy.mjs';
...
const QUERY_INDEFINITE_PATTERN = /(?:它|这个|这些|上面|上述|前者|后者|那个|其中|刚才)/u;
...
const EXHAUSTIVE_PATTERN = /(?:全部|所有|列出|穷举|一个不漏|出现在哪|出现于哪|哪些文件)/iu;
...
const SEARCH_COMMAND_PATTERN = /(?:请|帮我|查找|搜索|检索|列出|穷举|全部|所有|包含|提到|出现在哪(?:些)?|出现于哪(?:些)?|哪些文件|文件中|的文件)/gu;
...
function normalizeText(value) {
  return normalizeDates(String(value || '')
    .normalize('NFKC')
...
function exactTerms(query) {
  ...
  const dates = normalized.match(/\d{4}-\d{2}-\d{2}/gu) || [];
  const identifiers = normalized.match(/(?:[^\s"'“”`]+[/\\][^\s"'“”`]+|\b[A-Za-z_$][\w$]*(?:[._-][A-Za-z0-9_$-]+)+\b|\.[a-zA-Z0-9]{1,10}\b)/gu) || [];
  const strong = [...quoted, ...dates.map(normalizeText), ...identifiers.map(normalizeText)];
  if (strong.length) return [...new Set(strong)];
  return [...new Set(tokenize(normalized.replace(SEARCH_COMMAND_PATTERN, ' ')))]
    .sort((left, right) => right.length - left.length).slice(0, 12);
}
...
function contextualQuery(query, context = {}) {
  ...
  if (current.length >= 30 && !QUERY_INDEFINITE_PATTERN.test(current)) {
    return current.slice(0, QUERY_CONTEXT_LIMIT);
  }
  ...
  return [
    ...
    titles.length && `Previous answer titles: ${titles.join(' | ')}`,
  ...
  for (const [rank, item] of items.entries()) {
    ...
    if (existing) {
      ...
      existing.bestScore = Math.max(
        existing.bestScore,
        Number(item.rerankScore ?? item.rrfScore ?? item.vectorScore ?? item.bm25Score ?? item.score ?? 0),
      );
    ...
    selected.set(logicalKey, {
      ...
      bestScore: Number(item.rerankScore ?? item.rrfScore ?? item.vectorScore ?? item.bm25Score ?? item.score ?? 0),
    });
  }
...
export class KnowledgeIndex {
  ...
  async collectLiveChunks(signal) {
    const chunks = [];
    const paths = [];
    const indexed = this.chunksByPath(this.generation);
    const seen = new Set();
    ...
    for await (const relative of this.policy.walk()) {
      ...
      try {
        ...
        if (['VAULT_FILE_NOT_FOUND', 'VAULT_FILE_TOO_LARGE'].includes(error?.code)) continue;
        throw error;
      }
      seen.add(relative);
      paths.push(relative);
      const metadata = this.generation.files?.[relative];
      if (
        metadata && metadata.size === file.stat.size &&
        Math.trunc(metadata.mtimeMs) === Math.trunc(file.stat.mtimeMs) &&
        Math.trunc(metadata.ctimeMs) === Math.trunc(file.stat.ctimeMs)
      ) {
        chunks.push(...(indexed.get(relative) || []));
        continue;
      }
      hasChanges = true;
      const loaded = await this.readVaultFile(relative);
    ...
    if (Object.keys(this.generation.files || {}).some((relative) => !seen.has(relative))) hasChanges = true;
    return { chunks, paths, hasChanges };
  }

  async exactSearch(query, options = {}) {
    const terms = exactTerms(query);
    const live = await this.collectLiveChunks(options.signal);
    const indexedPaths = live.paths;
    ...
    if (!terms.length) {
      return { route: 'exact', exhaustive: Boolean(options.exhaustive), complete: Boolean(options.exhaustive),
        query: normalizeDates(query), results: [], diagnostics: {
          effectiveRoute: 'exact', generation: this.generation.generation,
          liveScan: true, liveOverlay: live.hasChanges, rerankerUsed: false,
        } };
    }
    const ranked = bm25Search(normalizeDates(query), live.chunks, live.chunks.length);
    const rankedByPath = new Map();
    for (const item of ranked) if (!rankedByPath.has(item.path)) rankedByPath.set(item.path, item);
    const results = [];
    for (const relative of indexedPaths) {
      options.signal?.throwIfAborted?.();
      const chunks = chunksByPath.get(relative) || [];
      ...
      for (const chunk of chunks) {
        ...
        hits.forEach((term) => contentHits.add(term));
        for (const match of lineMatches(chunk.content, terms)) {
          const absolute = chunk.lineStart + match.line - 1;
          if (!matches.some((item) => item.line === absolute && item.text === match.text)) {
            matches.push({ line: absolute, text: match.text });
          }
        }
      }
      if (!pathHits.length && !contentHits.size) continue;
      const best = rankedByPath.get(relative) || chunks[0] || {
        path: relative, name: path.basename(relative), heading: '', headings: [],
        lineStart: null, lineEnd: null, content: '',
      };
      results.push({ ...best, matches, lineNumbers: matches.map((match) => match.line),
        matchedTerms: [...new Set([...pathHits, ...contentHits])],
        score: pathHits.length * 20 + contentHits.size * 8 + matches.length +
          (Number(best.bm25Score) || 0) * 10 });
    }
    ...
    const limit = exhaustive ? results.length : Math.max(1, Math.min(30, Number(options.limit) || 30));
    return {
      route: 'exact', exhaustive, complete: exhaustive, query: normalizeDates(query),
      results: publicFileResults(results, query, limit, indexedPaths),
      diagnostics: { effectiveRoute: 'exact', generation: this.generation.generation,
        terms, liveScan: true, liveOverlay: live.hasChanges, rerankerUsed: false },
    };
  }
  ...
  async search(queryInput, options = {}) {
    ...
    const automatic = !String(options.route || '').trim();
    const routed = routeKnowledgeQuery(query);
    const requestedRoute = String(options.route || routed.route).trim().toLowerCase();
    if (!['keyword', 'semantic', 'hybrid', 'exact'].includes(requestedRoute)) {
      const error = new Error('Search route must be exact, keyword, semantic, or hybrid.');
      error.code = 'INVALID_SEARCH_ROUTE';
      error.status = 400;
      throw error;
    }
    if (requestedRoute === 'exact') {
      return this.exactSearch(query, { ...options, exhaustive: options.exhaustive ?? routed.exhaustive });
    }
    const taskMode = options.taskMode === 'deep' ? 'deep' : 'normal';
    const recallLimit = taskMode === 'deep' ? 60 : 30;
    const rrfLimit = taskMode === 'deep' ? 40 : 24;
    const resultLimit = requestedRoute === 'keyword' || !automatic
      ? Math.max(1, Math.min(30, Number(options.limit) || (taskMode === 'deep' ? 20 : 12)))
      : taskMode === 'deep' ? 20 : 12;
    const retrievalQuery = contextualQuery(query, options);
    const live = await this.collectLiveChunks(options.signal);
    const indexedPaths = live.paths;
    const diagnostics = {
      requestedRoute, effectiveRoute: requestedRoute, taskMode,
      generation: this.generation.generation, liveOverlay: live.hasChanges,
      embeddingProvider: this.signature.provider,
      embeddingEnabled: this.embeddingEnabled,
      embeddingUsed: false, queryVectorCacheHit: false,
      indexedChunks: live.chunks.length,
      embeddedChunks: live.chunks.filter((chunk) => Array.isArray(chunk.vector)).length,
      rerankerUsed: false, rerankerRequested: false,
    };
    if (!query) return { route: requestedRoute, query, results: [], diagnostics };
    const entityAnchor = explicitEntityAnchor(query);
    const retrievalChunks = entityAnchor
      ? live.chunks.filter((chunk) => chunkContainsEntity(chunk, entityAnchor))
      : live.chunks;
    ...
    const keyword = bm25Search(retrievalQuery, retrievalChunks, recallLimit);
    diagnostics.keywordCandidates = keyword.length;
    if (requestedRoute === 'keyword') {
      return { route: 'keyword', query: retrievalQuery,
        results: publicFileResults(keyword, retrievalQuery, resultLimit, indexedPaths), diagnostics };
    }
    if (entityAnchor && !retrievalChunks.length) {
      diagnostics.semanticCandidates = 0;
      return { route: automatic ? 'semantic' : requestedRoute, query: retrievalQuery, results: [], diagnostics };
    }
    ...
    if (!this.embeddingEnabled || !embeddedChunks.length) {
      ...
      return { route: automatic ? 'semantic' : 'keyword', query: retrievalQuery,
        results: publicFileResults(keyword, retrievalQuery, resultLimit, indexedPaths), diagnostics };
    }
    let semantic;
    try {
      const queryEmbedding = await this.queryVector(retrievalQuery, options.signal);
    ...
    if (requestedRoute === 'semantic' && !automatic && options.fuse !== true) {
      return { route: 'semantic', query: retrievalQuery,
        results: publicFileResults(semantic, retrievalQuery, resultLimit, indexedPaths), diagnostics };
    }
    const fused = reciprocalRankFusion(keyword, semantic).slice(0, rrfLimit);
    let candidates = publicFileResults(fused, retrievalQuery, rrfLimit, indexedPaths);
    const bm25Top = new Set(publicFileResults(keyword, retrievalQuery, 12, indexedPaths).map((item) => item.logicalKey));
    const vectorTop = new Set(publicFileResults(semantic, retrievalQuery, 12, indexedPaths).map((item) => item.logicalKey));
    const common = [...bm25Top].filter((key) => vectorTop.has(key));
    const topThreeBm25 = new Set(publicFileResults(keyword, retrievalQuery, 3, indexedPaths).map((item) => item.logicalKey));
    const topThreeConflict = !publicFileResults(semantic, retrievalQuery, 3, indexedPaths)
      .some((item) => topThreeBm25.has(item.logicalKey));
    const shouldRerank = common.length < 6 || topThreeConflict;
    diagnostics.commonTop12Files = common.length;
    diagnostics.rerankerRequested = shouldRerank;
    if (shouldRerank && candidates.length && typeof this.client.rerank === 'function') {
      try {
        const ranked = await this.client.rerank(retrievalQuery, candidates.map((candidate) => [
          `Path: ${candidate.path}`,
        ...
        diagnostics.rerankerUsed = true;
      } catch (error) {
        if (options.signal?.aborted) throw error;
        diagnostics.fallback = 'rrf';
        diagnostics.warning = String(error?.code || 'KNOWLEDGE_RERANK_FAILED');
      }
    }
    return { route: automatic ? 'semantic' : requestedRoute, query: retrievalQuery,
      results: candidates.slice(0, resultLimit), diagnostics };
  }

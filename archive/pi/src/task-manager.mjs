...
import {
  learningReviewLimits,
  resolveLearningReviewRequest,
...
import { markPublicMessage, publicError } from '../../../src/public-errors.mjs';
import { inspectVaultReplica } from '../../../src/vault-replica.mjs';
...
function retrievalContextFromConversation(conversation) {
  ...
  const answer = String(previousAssistant?.content || previousAssistant?.text || '');
  return {
    previousUserQuestion: String(previousUser?.content || previousUser?.text || '').slice(0, 300),
    previousAnswerTitles: [...answer.matchAll(/^#{1,6}\s+(.+)$/gmu)]
...
export class TaskManager {
  constructor(config, dependencies) {
    ...
    // Old releases stored Pi Agent checkpoints. They are retained for safe
    // cleanup only; request execution no longer enters an autonomous Agent
    // loop and therefore creates no new checkpoint or tool transcript.
    this.piAgent = dependencies.piAgent || null;
    this.tasks = new Map();
  ...
  async createTask(userId, body = {}) {
    ...
    const learningReview = kind === 'qa' ? resolveLearningReviewRequest(prompt, {
      now: Date.parse(taskCreatedAt),
      timeZone: this.config.timezone,
      previousReview: referencedConversation?.researchContext?.learningReview,
      history: referencedConversation?.messages,
    }) : null;
    // This is the original Second Mind evidence-review budget, not a different
    // UI mode and not an autonomous Agent policy.  The server owns the entire
    // read/search plan; every Pi invocation remains a single tool-free call.
    const executionTaskMode = learningReview ? Object.freeze({
      ...taskMode,
      timeoutMs: learningReviewLimits.timeoutMs,
      maxModelCalls: learningReviewLimits.maxTurns,
    }) : taskMode;
    ...
    const effectiveWebSearch = webSearch && !learningReview;
    const webSearchStatus = this.webSearch.publicStatus?.() || {};
    if (effectiveWebSearch && (webSearchStatus.enabled !== true || webSearchStatus.configured !== true)) {
  ...
  nextModelCall(task, purpose) {
    ...
    const maximum = Number(task.taskMode?.maxModelCalls);
    if (Number.isSafeInteger(maximum) && task.modelCallSequence > maximum) {
      throw taskError(
        429,
        `The ${task.taskMode.id} task reached its ${maximum}-call model budget.`,
  ...
  async run(task, persistedConversation) {
    ...
    let persisted = false;
    task.status = 'running';
    if (task.kind === 'qa' && !task.learningReview && this.config.research?.contextualizerEnabled === true) {
  ...
  async runQa(task, conversation) {
    if (task.learningReview) return this.runDeterministicLearningReview(task, conversation);
  ...
  async runDeterministicLearningReview(task, conversation) {
    const result = await runLearningReview({
      task,
      review: task.learningReview,
      index: this.taskIndex(task),
      maxContextChars: this.config.retrieval.maxContextChars,
      emit: (type, data) => this.emit(task, type, data),
      generate: (messages, callOptions = {}) => this.generateModel(task, 'learning_review_extract', messages, {
        signal: task.abortController.signal,
        ...this.generationOptions(task),
        maxOutputTokens: learningReviewLimits.extractionOutputTokens,
        timeoutMs: Math.min(learningReviewLimits.extractionTimeoutMs,
          Number(callOptions.timeoutMs) || learningReviewLimits.extractionTimeoutMs),
      }),
      generateFinal: (messages) => this.generateModel(task, 'learning_review_group', messages, {
        signal: task.abortController.signal,
        ...this.generationOptions(task),
        maxOutputTokens: learningReviewLimits.extractionOutputTokens,
        timeoutMs: Math.max(1, Math.min(learningReviewLimits.extractionTimeoutMs, task.deadlineAt - Date.now())),
      }),
      budgetAvailable: (reserveMs = 0) => (
        !task.abortController.signal.aborted &&
        Date.now() + Math.max(0, Number(reserveMs) || 0) < task.deadlineAt &&
        Math.max(0, Number(task.modelCallSequence) || 0) < learningReviewLimits.maxTurns
      ),
    });
    task.learningReviewCoverage = result.coverage;
    this.emit(task, 'text', { text: result.answer });
    conversation.messages.push({
      role: 'assistant', content: result.answer, at: new Date().toISOString(),
    });
    conversation.messages = conversation.messages.slice(-MAX_MESSAGES);
    conversation.researchContext = {
      ...(conversation.researchContext || {}),
      learningReview: task.learningReview,
    };
  }
  ...
  researchBudgetAvailable(task, additionalMs = 0) {
    ...
    const taskDuration = Math.max(1, Number(task.taskMode?.timeoutMs) || 10 * 60_000);
    // The original contract has one whole-task deadline (10m/30m). A provider
    // timeout may equal that deadline, so reserving the entire provider timeout
    // up front would suppress even the mandatory first local search. Reserve a
    // bounded quarter of the task unless the operator configured a smaller
    // explicit final reserve; the task AbortSignal remains the hard ceiling.
    const configuredReserve = Number(this.config.research?.finalReserveMs);
    const reserve = Number.isFinite(configuredReserve) && configuredReserve > 0
      ? Math.min(taskDuration - 1, Math.max(31_000, configuredReserve))
      : Math.min(llmReserve, Math.max(31_000, Math.floor(taskDuration / 4)));

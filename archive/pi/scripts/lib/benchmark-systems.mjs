...
function enforcingLlm(llm, { onCall = () => {} } = {}) {
  if (!llm || typeof llm.generate !== 'function') {
    throw systemError(
      'MigratedRagRunner requires an injected single-generation executor; implicit network access is disabled.',
      'LLM_REQUIRED',
    );
  }
  return Object.freeze({
    mapsRequestedEffort: llm.mapsRequestedEffort === true,
    requiresCompleteAssistantReplay: llm.requiresCompleteAssistantReplay === true,
    async generate(messages, options = {}) {
      const startedAt = performance.now();
      let usage = null;
      try {
        const output = await llm.generate(messages, {
          ...options,
          model: BENCHMARK_SYSTEM_MODEL,
          temperature: BENCHMARK_SYSTEM_TEMPERATURE,
          maxOutputTokens: Math.min(
            BENCHMARK_SYSTEM_MAX_OUTPUT_TOKENS,
            Number(options.maxOutputTokens) || BENCHMARK_SYSTEM_MAX_OUTPUT_TOKENS,
          ),
          onUsage: async (event = {}) => {
            if (event.usage) usage = cloneJson(event.usage);
            if (typeof options.onUsage === 'function') await options.onUsage(event);
          },
        });
        onCall({
          status: 'completed', durationMs: performance.now() - startedAt,
          effort: String(options.effort || BENCHMARK_SYSTEM_EFFORT), usage,
        });
        return output;
      } catch (error) {
        onCall({
          status: 'failed', durationMs: performance.now() - startedAt,
          effort: String(options.effort || BENCHMARK_SYSTEM_EFFORT), usage,
          errorCode: String(error?.code || 'LLM_CALL_FAILED'),
        });
        throw error;
      }
    },
    ...
    try {
      ...
      manager = new initialized.TaskManager(config, {
        ...
        llm: enforcingLlm(this.llm, {
          onCall: (call) => modelCalls.push({
            index: modelCalls.length,
            engine: 'pi-single-generation',
            model: BENCHMARK_SYSTEM_MODEL,
            effort: call.effort,
            temperature: BENCHMARK_SYSTEM_TEMPERATURE,
            maxOutputTokens: BENCHMARK_SYSTEM_MAX_OUTPUT_TOKENS,
    ...
    return {
      ...
      retrieval: {
        route: question.mode === 'deep'
          ? 'deep-hybrid'
          : searches.length > 1 ? 'server-multi-search' : searches[0]?.retrieval?.route || null,
      ...
      model: {
        ...
        usage: cloneJson(modelCalls.findLast((call) => call.usage)?.usage || null),
        agent: agentMetrics,
        telemetry,
      },
      timing: {

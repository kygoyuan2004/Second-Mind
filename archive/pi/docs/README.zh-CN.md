...
| 生成 | 服务端保留既有 Normal/Deep 有界流水线；Pi 只执行既定的单次模型调用；日记、计划、随心记草稿；SSE 进度和 Vault 相对路径引用 |
...
Second Mind 不是多租户 SaaS。Pi 不获得 shell、文件、网络或知识工具，也不自主追加模型回合；服务端固定用户、知识库、索引与 Provider 租约，执行检索、路径/哈希/覆盖/引用检查，并且只在草稿经用户确认后执行写入。
...
- 用户发起生成时，LLM 可接收问题、近期完整问答轮次、服务端选定的有界证据和文本附件片段；不发送工具 schema；
...
- [Pi 执行层迁移与运维](pi-agent-migration.md)：版本锁定、单次生成边界、更新和回滚

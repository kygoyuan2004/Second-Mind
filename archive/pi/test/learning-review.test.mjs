...
test('implicit personal review resolves a calendar month without a model clarification', () => {
  assert.equal(review.kind, 'learning_review');
...
test('actual partial progress survives an unchecked whole-task box, but future targets do not', () => {
  for (const text of [
    '- [ ] 阅读两个内核文件（计划2小时，今日完成1小时，剩余顺延）',
    '- [ ] 阅读部署指南（计划2小时，今日完成1.5小时，剩余顺延）',
  ]) {
    const [segment] = learningReviewSegments('plans/2026-08-31.md', text, review);
    for (const status of ['completed', 'in_progress', 'planned']) {
      const result = validateLearningReviewFacts({ facts: [proposedFact(segment, {
        statement: '阅读已有部分进度，剩余内容顺延。', status,
      })] }, [segment], review);
      assert.equal(result.facts[0]?.status, 'in_progress');
    }
    assert.equal(validateLearningReviewFacts({ facts: [proposedFact(segment, {
      statement: '已经完成整个阅读任务。',
    })] }, [segment], review).facts.length, 0);
  }
  const [planned] = learningReviewSegments('plans/2026-08-31.md',
    '- [ ] 计划今日完成1小时阅读，剩余部分明天处理。', review);
  assert.equal(validateLearningReviewFacts({ facts: [proposedFact(planned)] }, [planned], review).facts[0]?.status, 'planned');
});

test('explicit learning-and-documenting receipts count as completed, while planned and negated receipts do not', () => {
  for (const [text, expected] of [
    ['学习并整理了家庭网络相关信息，已记录到笔记中。', 'completed'],
    ['阅读并总结了检索算法。', 'completed'],
    ['计划学习并整理了相关信息。', 'planned'],
    ['尚未学习并整理了相关信息。', 'unconfirmed'],
  ]) {
    const [segment] = learningReviewSegments('diary/2026-08-25.md', text, review);
    assert.equal(validateLearningReviewFacts({ facts: [proposedFact(segment)] }, [segment], review).facts[0]?.status, expected, text);
  }
});
...
test('prior-plan references do not rewrite the current event and evidence dates stay source-local', () => {
  const [current] = learningReviewSegments('plans/2026-08-27.md',
    '- [ ] 训练 Orion 模型\n- 承接 2026-08-26「训练 Orion」的进度，今天继续训练。', review);
  const [later] = learningReviewSegments('plans/2026-08-31.md', '- [x] 结束 Orion 作业', review);
  const proposal = proposedFact(current, { statement: 'Orion 训练计划', status: 'planned' });
  proposal.evidence.push({ segmentId: later.id, lineStart: 1, lineEnd: 1, quote: later.text });
  const fact = validateLearningReviewFacts({ facts: [proposal] }, [current, later], review).facts[0];
  assert.deepEqual(fact.dateRange, { start: '2026-08-27', end: '2026-08-31' });
  assert.deepEqual(fact.evidence.map((item) => item.dateRange), [
    { start: '2026-08-27', end: '2026-08-27' }, { start: '2026-08-31', end: '2026-08-31' },
  ]);
});
...
test('carryover references retain today while an actual yesterday completion retains its explicit date', () => {
  for (const [text, expected, status] of [
    ['昨日（09-02）阅读剩余已并入今日；今日不安排下一项。', '2026-09-03', 'planned'],
    ['昨日（09-02）已完成 Orion 练习。', '2026-09-02', 'completed'],
  ]) {
    const [segment] = learningReviewSegments('plans/2026-09-03.md', text, review);
    const [fact] = validateLearningReviewFacts({ facts: [proposedFact(segment, { statement: text, status })] }, [segment], review).facts;
    assert.equal(fact.eventDate, expected);
  }
});

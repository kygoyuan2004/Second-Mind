...
test('authenticated HTTP API exposes and executes bounded Deep retrieval', async (t) => {
  ...
  assert.equal(fusion?.data.diagnostics.strategy, 'multi-query-mixed-rrf');
  assert.equal(searches.length, 3);
  assert.deepEqual(searches.map((item) => item.options.taskMode), ['deep', 'deep', 'deep']);
  assert.ok(searches.every((item) => item.options.route === undefined));

...
test('fake upstream integration runs public probe then four private paired calibrations',
  { timeout: 60_000 }, async (t) => {
    ...
    assert.deepEqual(
      ...
      {
        'public-probe': 1,
        // The fake upstream classifies only by tool schemas. Tool-free Pi
        // generation is intentionally indistinguishable from a plain
        // Anthropic request at that boundary.
        'original-agent': 8,
        'pi-capability': 0,
        'pi-knowledge': 0,
      },
    ...
    assert.equal(telemetryWindows.at(-1).cursorEnd, 9);

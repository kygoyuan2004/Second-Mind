...
| 生成 | 内嵌 Pi Agent 受限工具循环；Normal 与 Deep 使用不同有界预算；日记、计划、随心记草稿；SSE 进度和带 Vault 相对路径的引用 |
...
Second Mind 不是多租户 SaaS。模型不会获得 shell、任意文件读取或任意网络工具。Pi 根据每次结果自主选择下一项受限知识工具；服务端固定用户、知识库、索引与 Provider 租约，执行路径/哈希/覆盖/引用检查，并且只在草稿经用户确认后执行写入。
...
- 用户发起生成时，LLM 可接收问题、近期完整问答轮次、受限工具 schema、Pi 自主选择的有界工具结果和文本附件片段；
- 管理员确认构建向量索引或执行语义查询时，Embedding 服务可接收笔记文本块或查询；
- 当前问答会话启用联网补充时，Pi 可在任何 Vault 结果暴露前调用 WebSearch；WebSearch 可接收有限查询词，之后搜索调用永久关闭；
- Pi 对同一任务搜索结果中的精确 URL 调用 `web_read` 时，安全读取器可请求经过验证的 HTTPS 页面，并把定界后的正文交给模型。个人学习回顾不提供联网工具。
...
- [Pi Agent 迁移与运维](pi-agent-migration.md)：版本锁定、工具边界、事务化会话、更新和回滚
- [个人学习回顾](learning-review.md)：固定日期范围、清单分页、原文读取与覆盖门禁

...
- “多智能体协同”“ReAct Agent”“自主调用工具”——当前 Deep 是服务端有界的
  provider-neutral Deep Retrieval；Pi 只执行流水线已计划的单次生成；
...
当前是单进程，BM25 查询仍会遍历内存 chunk，但文档 tokenization 已按
chunk 内容缓存，重复查询不会重新分词；索引和会话是本地 JSON，任务与
限流是内存状态。因此主要瓶颈仍是大语料检索复杂度、

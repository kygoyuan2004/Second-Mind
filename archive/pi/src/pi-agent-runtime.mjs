...
import { isInside } from '../../../src/path-policy.mjs';
...
function knowledgeSystemPrompt(input = {}) {
  ...
  return [
    ...
    review
      ? 'Learning reviews are owned by the deterministic server orchestrator and must not execute in this legacy Agent runtime.'
      : input.taskMode === 'deep'
...
function sessionSettings(config, timeoutMs, contextWindow) {
  const windowTokens = Math.max(
    MIN_PI_CONTEXT_WINDOW_TOKENS,
    Number(contextWindow) || DEFAULT_PI_CONTEXT_WINDOW_TOKENS,
  );
...
export class PiAgentRuntime {
  ...
  async runQa({ task, conversation, indexSnapshot, emit, prompt = task.prompt }) {
    if (task.learningReviewRequest) {
      throw piError(
        'Learning reviews must use the deterministic server orchestrator.',
        'PI_AGENT_LEARNING_REVIEW_DENIED',
        500,
      );
    }
  ...
  async #run({
    ...
    let limitError = null;
    let completed = false;
    const maxAgentTurns = tools.length ? task.taskMode.id === 'deep' ? 24 : 12 : 2;
    const maxAgentToolCalls = tools.length ? task.taskMode.id === 'deep' ? 32 : 16 : 0;

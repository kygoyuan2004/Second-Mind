...
function offlinePiFixture(options = {}) {
  const observations = { generateCalls: [], runs: [] };
  const llm = {
    mapsRequestedEffort: true,
    async generate(messages, generationOptions = {}) {
      const prompt = messages.map((message) => String(message.content || '')).join('\n');
      observations.generateCalls.push({ prompt, options: { ...generationOptions } });
      if (prompt.includes('Return JSON only in the exact shape {"queries"')) {
        return JSON.stringify({ queries: [
          'Project Aurora launch timing',
          'Project Aurora owner',
        ] });
      }
      await options.afterGenerate?.({ messages, generationOptions });
      const date = prompt.match(/\b\d{4}-\d{2}-\d{2}\b/u)?.[0] || 'unknown';
      const owner = prompt.match(/owner is ([A-Za-z]+)/iu)?.[1] || '';
      const cited = prompt.match(/<source[^>]+path="([^"]+)"/u)?.[1] || 'notes/aurora.md';
      const answer = `The launch date is ${date}${owner ? ` and the owner is ${owner}` : ''}. [[${cited}]]`;
      generationOptions.onToken?.(answer);
      await generationOptions.onUsage?.({
        phase: 'final', protocol: 'fixture', stopReason: 'stop',
        usage: {
          inputTokens: 120, outputTokens: 24, cacheReadInputTokens: 0,
          cacheCreationInputTokens: 0, reasoningTokens: 0, totalTokens: 144,
        },
      });
...
test('MigratedRagRunner keeps deterministic retrieval and uses only single-generation calls', async (t) => {
  const project = await fixture();
  t.after(project.cleanup);
  const offline = offlinePiFixture();
  const runner = new MigratedRagRunner({
  ...
  assert.equal(normal.model.calls[0].engine, 'pi-single-generation');
  assert.equal(normal.model.calls[0].temperature, 0);
  assert.equal(normal.model.turns, 1);
  assert.equal(normal.model.agent, null);
  assert.equal(normal.model.usage.totalTokens, 144);
  assert.equal(normal.coverage, null);
  assert.ok(normal.toolEvents.some((event) => event.toolName === 'vault_search'));
  ...
  assert.equal(offline.observations.generateCalls.length, 1);
  assert.match(offline.observations.runs[0].prompt, /2026-09-05/u);
  assert.match(offline.observations.runs[0].prompt, /We were discussing/u);
